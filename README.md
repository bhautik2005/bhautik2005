# 👋 Hi, I'm Bhautik Gondaliya

🚀 AI/ML Engineer | Data Science Enthusiast | Backend Developer
💡 Building intelligent systems, scalable APIs, and real-world applications

---

## 🧠 About Me

* 🎓 Engineering Student (GTU - Semester 6)
* 🤖 Passionate about **AI, Machine Learning, NLP & Backend Systems**
* 🛠️ Skilled in building **end-to-end ML applications + REST APIs**
* 🌱 Currently exploring **Deep Learning, LLMs & Production AI Systems**

---

## ⚡ Tech Stack

### 👨‍💻 Programming

`Python` `JavaScript` `SQL`

### ⚙️ Backend & Full Stack

`Flask` `Node.js` `Express` `MongoDB` `MERN Stack` `REST APIs`

### 🤖 AI / ML / Data Science

`NumPy` `Pandas` `Scikit-learn` `NLP` `LSTM`
`EDA` `Feature Engineering` `Data Visualization`

### 🛠️ Tools

`Git` `GitHub` `Jupyter` `Colab` `Tableau` `VS Code`

---

## 🔥 Featured Projects

### ❤️ Heart Disease Prediction

🔗 [Live App](http://heart-react-app.s3-website.ap-south-1.amazonaws.com)
🔗 [GitHub](https://github.com/bhautik2005/heart_disease_project)

* ML model predicting heart disease using clinical data
* Flask API + frontend integration
* Achieved **[X]% accuracy**

---

### 🧠 Next Word Predictor (LSTM)

🔗 [GitHub](https://github.com/bhautik2005/Next_Prediction-LSTM-)

* Deep Learning NLP model using LSTM
* Sequential text prediction system
* Real-time inference using Flask

---

### 🏠 Nest – Booking Platform

🔗 [Live App](http://Nest-env.eba-xvf3vp3w.ap-south-1.elasticbeanstalk.com)
🔗 [GitHub](https://github.com/bhautik2005/Nest)

* Full-stack MERN application
* Authentication, booking system, REST APIs
* Scalable backend architecture

---

### 📄 AI PDF Chatboard

🔗 [GitHub](https://github.com/bhautik2005/AI_pdf_chatboard)

* Chat with PDFs using AI
* Semantic search + embeddings
* Efficient document understanding system

---

## 📊 GitHub Stats

![Bhautik's GitHub stats](https://github-readme-stats.vercel.app/api?username=bhautik2005&show_icons=true&theme=tokyonight)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=bhautik2005&layout=compact&theme=tokyonight)

---

## 📫 Connect With Me

* 💼 [LinkedIn](https://www.linkedin.com/in/bhautik-gondaliya-310181370/)
* 📧 [bhautik613@gmail.com](mailto:bhautik613@gmail.com)

---

⭐ *If you like my work, consider giving a star to my repositories!*
